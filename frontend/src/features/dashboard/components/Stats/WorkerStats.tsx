...
export function WorkerStats() {
    ...
    const copyToClipboard = async (text: string) => {
        try {
            ...
            console.log('Successfully copied:', text) // Для отладки
            setIsCopied(true)
            setTimeout(() => setIsCopied(false), 2000)
        } catch (err) {
            console.error('Failed to copy text:', err)
            // Даже при ошибке показываем обратную связь для тестирования
            setIsCopied(true)
            setTimeout(() => setIsCopied(false), 2000)
        }
    }
        ...
        <div className="bg-[#222222] border border-border rounded-xl p-6 h-full flex flex-col">
            ...
            <div className="mt-auto">
                ...
                <div className="text-text-muted text-sm mb-2">
                    To try it out, connect your miner to this address:
                </div>
                <div className="flex items-center gap-2 p-2 bg-[#2A2A2A] rounded-md">
                    {/* Via Icon */}
                    ...
                    {/* Address */}
                    <span className="text-text-white text-sm flex-1">miner.com:3333</span>

                    {/* Copy button */}
                    <button
                        onClick={() => {
                            console.log('Copy button clicked!') // Для отладки
                            copyToClipboard('miner.com:3333')
                        }}
