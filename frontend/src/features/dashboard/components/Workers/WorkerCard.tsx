...
export function WorkerCard({ worker }: WorkerCardProps) {
  const statusColors = {
    ...
    unactive: 'text-red-500'
  }

  return (
      <div className="bg-[#222222] rounded-xl p-4 hover:border-accent-green/30 transition-all duration-300 group">
          {/* Header с статусом */}
          <div className="flex items-center gap-3 mb-3 ">
              <div className="flex items-center gap-2 ">
          <span
              className={`text-xs bg-[#FFFFFF12] text-[#00FF26] p-2 rounded-md font-medium ${statusColors[worker.status]}`}>
            {worker.status === 'active' ? 'Active' : 'Unactive'}
          </span>
              </div>

              {worker.customFee !== 'none' && (
                  <div className="flex items-center gap-1 bg-[#FFFFFF12] rounded-md p-2">
            <span className="text-xs font-medium text-purple-400">
              {worker.customFee} custom fee
            </span>
                  </div>
              )}
          </div>
          ...
          {/* Хешрейт */}
          <div className="text-text-primary text-2xl font-bold mb-4">
              {worker.hashrate}
              <span className="text-xl font-bold ml-1">TH/s</span>
          </div>
          <div className="h-px w-full bg-[#313131]"></div>
              ...
              <div className="flex items-center gap-2">
                  <button
                      ...
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <g clip-path="url(#clip0_537_28246)">
                              <path
                              ...
                              <path d="M10 13.3333V10" stroke="#7A7A7A" stroke-width="1.5" stroke-linecap="round"
                                    stroke-linejoin="round"/>
                              <path d="M10 6.6665H10.0088" stroke="#7A7A7A" stroke-width="1.7" stroke-linecap="round"
                                    stroke-linejoin="round"/>
                          </g>
                          <defs>
                              <clipPath id="clip0_537_28246">
